...
const NEWDM = () => {
  const { selectedChatData, selectedChatType, setSelectedChatType, setSelectedChatData } = useAppStore();
  ...
  return (
   ...
   <TooltipProvider>
     <Tooltip>
       <TooltipTrigger><FaPlus onClick={()=>setOpenNewContactModal(true)}
  ...
  <DialogContent className="bg-[#181920] border-none text-white w-[400px] h-[400px] flex flex-col">
             ...
             <div className='w-12 h-12 relative '>
                         <Avatar className='h-12 w-12 rounded-full overflow-hidden'>
                            {contact.image ? (
                              ...
                              <div
                                className={`uppercase h-12 w-12 text-2xl border-[1px] flex items-center justify-center rounded-full ${getColor(contact.color)}`}
                              >
                                {contact.firstName
    ...
    {
      searchedContacts.length<=0 && 
      ( <div className='flex-1  md:flex mt-2 md:mt-0 flex-col justify-center items-center hidden duration-1000  transition-all'>
      <Lottie
